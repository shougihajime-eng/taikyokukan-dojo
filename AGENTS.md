...
- ✅ 済み: コアループ（出題→スライダー→判定→テンプレ解説→集計）／かんたんログイン／クセ分析ダッシュボード／先生ページ／インポート受け皿／かり局面12個投入／本番公開（hnd1確認済）／単体テスト42件・通しテスト22項目合格／**解説の強化（2026-06-08）**: 駒の損得の機械数え（lib/material.ts）＋「形勢判断の4つのものさし」まめちしき（components/MasterTips.tsx・「やさしい／くわしい」切り替え付き＝選択を端末に保存）を答え合わせ画面に追加
