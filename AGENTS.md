...
- ✅ **解説に「なぜ？」の一言を追加（lib/explain.ts の whyText）**: 駒の損得の機械数え＋形勢の大きさ＋一般的なものさし（玉の堅さ/駒の働き/手番）だけから組み立てる安全文（ハルシネーション禁止を維持）。ResultReveal に朱の囲みで表示。⚠**コードは未コミット（未公開）**＝上のデザイン刷新と同じバンドルで本人レビュー後に公開
- 🟡 進行中: なし（150問は本番で稼働中）
- 🔜 次の一歩: ①未公開コード（世界観デザイン＋whyText＋generate-positions-v2＋/api/import の purgeSamples）を本人レビュー → OKなら main へ push で公開（whyの一言もそこで本番反映） ②本人が問題をプレイして「これは出したくない」局面があれば外す（DBの positions から該当行を削除するだけ＝元データは scripts/out に保管） ③数を増やすときは `node scripts/generate-positions-v2.mjs`（ShogiHome等を閉じてから）→ `import-positions.mjs` で追加投入
